    ...
    <div style={{ width: 220, background: '#1f2937', color: 'white', padding: 20 }}>
      ...
      {user && (
        <div className="space-y-4">
          ...
          {user.role === 'student' && <Link to="/student" className="block hover:underline">My Dashboard</Link>}
          {user.role === 'teacher' && <Link to="/teacher" className="block hover:underline">Teacher Dashboard</Link>}
          {user.role === 'admin' && <Link to="/admin" className="block hover:underline">Admin Dashboard</Link>}
          <hr className="border-gray-500" />
          <button
            onClick={logout}
