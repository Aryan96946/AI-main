...
export default function Login({ onLogin }) {
  ...
  const currentEmailInput = loginMode === 'otp' && otpStep === 1 ? emailRef : emailRef;
          ...
          >
            ...
            <div className="flex bg-white/10 rounded-xl p-1 mb-6">
              <button
                ...
                className={`flex-1 py-2 px-4 rounded-lg font-medium transition-all ${loginMode === 'password' ? 'bg-emerald-500 text-white shadow-lg' : 'text-gray-300 hover:text-white'}`}
              >
                Password
              </button>
              <button
                ...
                className={`flex-1 py-2 px-4 rounded-lg font-medium transition-all ${loginMode === 'otp' ? 'bg-blue-500 text-white shadow-lg' : 'text-gray-300 hover:text-white'}`}
              >
                OTP
              </button>
            </div>
            ...
            <motion.div {...fadeInUp}>
              <div className="space-y-5">
                ...
                {loginMode === 'otp' && otpStep === 2 && (
                  <div>
                    <label className="block text-gray-300 text-sm font-medium mb-2" htmlFor="otp">OTP Code</label>
                    <motion.input
                      ref={otpRef}
                      id="otp"
                      type="text"
                      ...
                      onKeyDown={handleKeyDown}
                      className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-center text-lg tracking-widest font-mono"
                      whileFocus={{ scale: 1.02 }}
                      aria-label="OTP code"
                    />
                    {countdown > 0 && (
                    ...
                    {countdown === 0 && otpSent && (
                      <button
                        onClick={resendOtp}
                        className="text-xs text-blue-400 hover:text-blue-300 mt-1 w-full font-medium"
                      >
                        Resend OTP
                      </button>
                    )}
                ...
                <motion.button
                  ...
                  style={{ background: loginMode === 'otp' ? 'linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)' : 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white' }}
                >
                  {loading ? (
                    <>
                      ...
                      {loginMode === 'password' ? 'Signing in...' : otpStep === 1 ? 'Sending OTP...' : 'Verifying...'}
                    </>
                  ) : loginMode === 'password' ? 'Sign In' : otpStep === 1 ? 'Send OTP' : 'Verify OTP'}
                </motion.button>
              </div>
            </motion.div>
