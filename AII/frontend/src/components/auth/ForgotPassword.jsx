...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!email) return setErr("Email or username is required");
    ...
    try {
      ...
      setErr(error.response?.data?.error || "Failed to send reset link");
    } finally {
      setLoading(false);
    }
  };
            ...
            {!success ? (
              <motion.form onSubmit={handleSubmit} {...fadeInUp}>
                <div className="space-y-5">
                  <div>
                    <label className="block text-gray-300 text-sm font-medium mb-2">Email Address or Username</label>
                    <motion.input
                      type="text"
